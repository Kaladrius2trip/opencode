...
describe("AnthropicAuthPlugin", () => {
  test("refresh falls back to console token endpoint with form body", async () => {
    ...
    const state: { id?: string; access?: string; refresh?: string; accountId?: string } = {}
    ...
    const plugin = await AnthropicAuthPlugin({
      client: {
        auth: {
          set: async (input: {
            path: { id: string }
            body: { access: string; refresh: string; accountId?: string }
          }) => {
            state.id = input.path.id
            state.access = input.body.access
            state.refresh = input.body.refresh
            state.accountId = input.body.accountId
          },
        },
      },
      project: {},
    ...
    const get = Object.assign(
      async () => ({ type: "oauth" as const, access: "", refresh: "refresh-1", expires: 0, accountId: "acct-1" }),
      { providerID: "anthropic-work" },
    )

    const cfg = await auth.loader(get, {
      models: { claude: {} },
  ...
  test("refresh retries once after transient 400 failure", async () => {
    const calls: string[] = []
    const state: { access?: string; refresh?: string } = {}
    let count = 0

    globalThis.fetch = (async (req: Request | string | URL) => {
      const url = req instanceof Request ? req.url : req.toString()
      calls.push(url)

      if (url === "https://claude.ai/oauth/token") {
        count += 1
        if (count === 1) return new Response("temporary failure", { status: 400 })
        return Response.json({
          refresh_token: "refresh-2",
          access_token: "access-2",
          expires_in: 3600,
      ...
      if (url === "https://console.anthropic.com/oauth/token" && count === 1) {
        return new Response("temporary failure", { status: 400 })
      }
      return Response.json({ ok: true })
    }) as typeof fetch
  ...
  test("refresh surfaces reconnect message for expired oauth", async () => {
    globalThis.fetch = (async (req: Request | string | URL) => {
      const url = req instanceof Request ? req.url : req.toString()
      if (url.includes("/oauth/token")) {
        return new Response('{"error":"invalid_grant","error_description":"refresh token expired"}', {
          status: 400,
    ...
    const run = cfg.fetch!("https://api.anthropic.com/v1/messages", {
      method: "POST",
      body: "{}",
      headers: {
        "x-api-key": "dummy",
    ...
    return expect(run).rejects.toThrow("Anthropic OAuth expired")
  })
})
